...
| `src/store.rs` | 源解析、Rust crate 校验、编译安装、列举、卸载、进程调用 |
...
编译失败时清理临时目录；成功后使用同文件系统目录重命名发布。拒绝同名覆盖，并发安装只有一个成功；进程被强制杀死时可能留下隐藏的 `.install-*` 临时目录，可在确认没有安装任务后删除。
不支持安装过程中修改源码或同时卸载正在运行的插件。

```text
DM_HOME/
├── registry.toml              # 可选名称 -> HTTPS Git 地址
└── plugins/
    └── hello/
        ├── dm-plugin.toml
        └── dm-hello[.exe]
...
源解析集中在 `PluginStore::install`，后续可以接入受维护的注册表、固定 Git revision、签名和二进制分发。首版只获取远程默认分支，`Cargo.lock` 固定依赖但不固定远程插件源码；需要固定版本时在本地检出对应提交再安装。
